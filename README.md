...
- 图标为空时自动抓取并保存为本地 WebP；支持手动获取、图片加载失败回退，以及 HTTP 环境中的资源、分类和标签 ID 创建。
- 批量导入浏览器书签 HTML、CSV/TSV 或多行网址；导入前预览、跳过重复和无效项，并复用或新增分类、标签。
...
在后台“应用”或“书签”页面编辑资源，修改“资源类型”后保存即可互转。也可以勾选多行，点击“转为书签”或“转为应用”并确认。转移只改变资源类型，保留原有 ID、名称、网址、描述、图标、分类、标签和在资源数组中的顺序，列表和侧栏数量会立即更新。

“全选当前结果”和“反选”只作用于当前搜索结果；搜索后隐藏的资源会取消勾选，切换后台页面会清空选择。批量转移与批量删除都会先显示确认提示，并通过现有修订号检查保存；另一窗口已更新数据时会提示重新加载，失败时保留当前勾选。

“批量导出”支持四种范围：已选资源、当前筛选结果、当前页面全部资源、全部应用和书签。不必先勾选资源即可导出当前页面或全部资源，弹窗会显示导出数量。

- **JSON**：保留每个资源的完整字段，并附带分类名称和标签名称，便于后续处理。
- **CSV**：导出类型、名称、网址、描述、分类、标签和图标地址；使用 UTF-8 BOM 支持表格软件读取中文，逗号、引号和换行会正确转义，公式形式的字段按普通文本处理。
- **浏览器书签 HTML**：应用和书签均可导出为浏览器书签，按分类生成文件夹，支持导入 Chrome、Edge、Firefox；名称、链接与标签中的 HTML 特殊字符会转义。

资源导出中的图标保留原地址，不打包图片文件；资源 JSON 也不是整站恢复文件。迁移本站图片、主题和站点配置时，请使用“备份与迁移”中的完整 JSON 备份。

## 图标与网址

添加或编辑资源时，裸域名（如 `example.com`）会在失焦、手动获取图标和保存时补全为 `https://example.com`；已有的 HTTP(S) 协议保留。后端仍校验协议、账号密码和字段长度。

图标地址留空时，保存会自动尝试网站自身 `/favicon.ico`、Google Favicon、DuckDuckGo Favicon；也可点击“手动获取”先回填，或使用原有上传按钮。成功后转换为 `/uploads/` 下的 WebP，并可随站点备份导出；支持常见栅格格式以及包含 PNG/位图的 ICO。获取失败或超时不阻止保存，手动获取失败也不会清空已有图标。

首页依次尝试已保存图标、内置图标、网站自身 `/favicon.ico`、Google Favicon、DuckDuckGo Favicon；全部失败时显示名称首字。HTTP 环境下创建应用、书签、分类和标签可自动降级到兼容的 ID 生成方式。

`POST /api/admin/favicon` 接受 JSON `{ "url": "https://example.com" }`，需要管理员会话和现有的 `x-navigation-request: 1` 请求头。成功响应 `{ "url": "/uploads/…webp" }`，抓取失败响应 `{ "url": "" }`；非法输入仍返回 400，未登录返回 401。抓取仅访问公网 HTTP(S) 地址，每次跳转重新校验并连接已验证的 IP；私网资源仍可保存，但不会由服务器探测其图标。每个来源最多等待 3 秒、允许 3 次跳转，输入最多 2 MB / 100 万像素。图标接口独立限流为每分钟 120 次，避免挤占资源保存接口额度。

## 批量导入

在“应用”或“书签”页面点击“批量导入”，选择文件或粘贴内容，选择导入类型，再预览和确认。

- 浏览器书签 HTML：支持 Chrome、Edge、Firefox 等导出的 Netscape 书签格式。使用最近一层文件夹名称作为平面分类，同名分类合并；标签可从书签的 `TAGS` 属性读取。HTML 内容不会在页面中执行。
- CSV/TSV：必须包含 `url` 或“网址”列；可选 `name`/名称、`description`/描述、`category`/分类、`tags`/标签。支持引号、逗号和字段内换行，多个标签用分号分隔。
- 多行网址：每行一个网址，名称默认采用域名，支持裸域名补全。

CSV 示例：

```csv
名称,网址,描述,分类,标签
示例网站,example.com,常用工具,工作,效率;收藏
开发文档,https://developer.mozilla.org,Web 参考资料,开发,阅读
```

导入只追加资源。同一类型内，规范化后相同的网址会跳过；应用和书签可以分别收藏同一网址。预览显示可导入、重复和无效项及原因，确认时仅保存有效项。每批最多 2000 项、文件最多 5 MB，站点总上限仍为 2000 个资源、100 个分类、200 个标签。多人或多窗口修改冲突时，需要重新预览。

默认尝试获取新增资源图标，同一站点复用抓取结果，最多 3 个并发请求；整批图标获取最多等待 12 秒，超时或触及图标限流后仍保存资源。可取消“自动获取图标”以快速导入，之后在单项编辑中获取或上传。原来的 JSON“恢复备份”仍用于完整站点恢复。
...
`test:resources` 验证单个与批量互转、筛选与选择范围、三种格式的实际下载、修订号冲突、批量删除，以及 320/375/768 像素宽度下的布局。

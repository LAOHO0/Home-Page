...
可管理应用、分类书签和四套差异化主题的个人导航站。前端使用原生 JavaScript，后端为 Express + Node.js 内置 SQLite，排序使用 SortableJS，图标来自 Lucide。
...
- 经典圆润、晴空留白、终端石墨、山岚沉浸四套主题；分别采用圆润深色卡片、明亮双栏、紧凑列表和居中山景布局。每套分别保存整页、今日概览和资源区域背景。
...
- JSON 备份上限为 35 MB；超限时导出会提示压缩或减少图片，不生成无法恢复的文件。支持全部 2000 个资源图标和 13 个站点背景/Logo 图片。旧三主题备份导入时自动补齐经典主题与布局默认值。
- 源码目录 `src/`、SQLite、会话和项目文件不通过静态服务器暴露。

import { themeNames, filterEntries } from './model.js';
import { $, icon, installIcons, request, textElement, applyAppearance, makeResourceCard, storageGet, storageSet, showToast, cityPicker } from './ui.js';
...
function setTheme(theme) {
  ...
  $('#themes').querySelectorAll('button').forEach(button => button.setAttribute('aria-pressed', String(button.dataset.theme === theme)));
}
function populate() {
  ...
  const themes = $('#themes'); themes.replaceChildren();
  Object.entries(themeNames).forEach(([value, name]) => { const button = document.createElement('button'); button.type = 'button'; button.className = 'theme-swatch'; button.dataset.theme = value; button.textContent = { classic: '经典', sky: '晴空', graphite: '终端', forest: '山岚' }[value]; button.title = name; button.setAttribute('aria-label', name); button.onclick = () => { storageSet('navigation-theme', value); setTheme(value); }; themes.append(button); });
  const stored = storageGet('navigation-theme'); setTheme(themeNames[stored] ? stored : documentData.settings.theme);
...
function render() {
  ...
  if (!entries.length) { content.append(textElement('p', query || activeTag || $('#category-filter').value ? '没有找到匹配的资源' : '暂无资源', 'empty')); return; }
  if (type === 'bookmarks') {
    [...documentData.categories, { id: '', name: '未分类' }].forEach(category => {
      ...
      content.append(textElement('h3', category.name, 'group-title')); const grid = textElement('div', '', 'resource-grid'); group.forEach(x => grid.append(makeResourceCard(x))); content.append(grid);
    });
  } else { const grid = textElement('div', '', 'resource-grid'); entries.forEach(x => grid.append(makeResourceCard(x))); content.append(grid); }
}
...
$('#clear-search').onclick = () => { $('#query').value = ''; activeTag = ''; $('#category-filter').value = ''; render(); $('#query').focus(); };
